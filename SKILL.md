...
Read `references/article-writing-guide.md` from this skill directory before dispatching.
...
Construct the following prompt for each item (substitute all placeholders with actual values):
...
## 前景展望
## 社区讨论
← Only include if comments data was provided above; otherwise omit entirely.

---
*Generated by vv-horizon-briefing on {generated_at}*

**Writing guide (follow strictly):**
- 摘要: what + why it matters, ≤3 sentences, don't restate the title
- 核心内容: explain what happened/was built/discovered; include key data points, version numbers, feature lists; assume technical reader unfamiliar with this specific domain
- 技术背景: prerequisite knowledge, why the problem is hard, limitations of prior art; ≤200 words
- 方法与实现: core technical decisions, architecture choices, comparison with prior art, key innovations
- 影响与意义: short-term (who benefits immediately), long-term (industry direction); be specific, avoid vague claims
- 前景展望: open problems, next likely directions, risks and limitations
- 社区讨论: synthesize key viewpoints, identify disagreements and consensus, quote ≤3 representative comments in blockquote format
